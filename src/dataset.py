...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # 0. Retrieve the document and strip extra whitespace.
        doc = self.data[idx].strip()
        # If the document is very short, we use the whole document.
        if len(doc) < 4:
            truncated = doc
        else:
            # 1. Randomly truncate the document to a length between 4 and int(block_size*7/8)
            max_trunc = min(len(doc), int(self.block_size * 7 / 8))
            trunc_length = random.randint(4, max_trunc)
            truncated = doc[:trunc_length]
        
        L = len(truncated)
        # 2. Randomly choose a masked segment.
        # Here, we aim for a masked segment length around L/4 on average,
        # but choose uniformly between 1 and L//2.
        masked_len = random.randint(1, max(1, L // 2))
        # Ensure the masked segment fits in the truncated text.
        start = random.randint(0, L - masked_len)
        prefix = truncated[:start]
        masked_content = truncated[start:start+masked_len]
        suffix = truncated[start+masked_len:]
        
        # 3. Rearrange substrings into the following form:
        #    [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] [pads]
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        
        # Pad masked_string with PAD_CHAR to ensure its length is block_size + 1.
        ...
        # 4. Create input-output pair: x is masked_string[:-1] and y is masked_string[1:].
        x_str = masked_string[:-1]
        y_str = masked_string[1:]
        
        # 5. Encode characters into tensor indices.
