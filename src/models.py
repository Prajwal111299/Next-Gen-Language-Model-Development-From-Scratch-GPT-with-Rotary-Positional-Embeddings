...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        # On MPS: perform the embedding lookup on CPU then bring the result back to MPS.
        if idx.device.type == "mps":
            # Ensure the weight is on CPU
            cpu_weight = self.tok_emb.weight.to("cpu")
            # Perform embedding lookup on CPU using F.embedding
            token_embeddings = torch.nn.functional.embedding(
                idx.to("cpu"),
                cpu_weight,
                self.tok_emb.padding_idx,
                self.tok_emb.scale_grad_by_freq,
                self.tok_emb.sparse
            ).to(idx.device)
        else:
            token_embeddings = self.tok_emb(idx)

        # Compute input to the transformer: use RoPE or add positional embeddings.
        if self.rope:
